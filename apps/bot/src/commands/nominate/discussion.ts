...
export async function handleDiscussionCommand(
  ...
  try {
    ...
    const newVoteStart = new Date(
      discussionStart.getTime() + hours * 60 * 60 * 1000
    );
    ...
    if (newVoteStart <= currentTime) {
      ...
      const jobScheduler = NominationJobScheduler.getInstance(
        interaction.client
      );
    ...
    const newCleanupStart = new Date(
      newVoteStart.getTime() +
        NOMINATION_CONFIG.VOTE_DURATION_MINUTES * 60 * 1000
    );
    ...
    if (newVoteStart > oldVoteStart) {
      ...
      if (nextNominee) {
        ...
        const baseTime = new Date(
          newCleanupStart.getTime() +
            NOMINATION_CONFIG.CLEANUP_DURATION_MINUTES * 60 * 1000
        );
        ...
        for (let i = 0; i < queuedNominees.length; i++) {
          ...
          const newDiscussionStart = TimeCalculationService.getNextMondayAt9AM(
            new Date(baseTime.getTime() + weekOffset)
          );
          const newQueuedVoteStart = new Date(
            newDiscussionStart.getTime() +
              NOMINATION_CONFIG.DISCUSSION_DURATION_MINUTES * 60 * 1000
          );
          const newQueuedCleanupStart = new Date(
            newQueuedVoteStart.getTime() +
              NOMINATION_CONFIG.VOTE_DURATION_MINUTES * 60 * 1000
          );
    ...
    if (nominee.discussionChannelId) {
      ...
      if (discussionChannel?.isTextBased()) {
        ...
        if (embedMessage) {
          const discussionStart = new Date(nominee.discussionStart);
          const totalDuration =
            newVoteStart.getTime() - discussionStart.getTime();
