...
export class ChannelManagementService {
  ...
  async createVoteChannel(nominee: Nominee): Promise<ChannelCreationResult> {
    try {
      ...
      const requiredQuorum = Math.ceil(
        memberCount * ConfigService.getVoteQuorumPercent()
      );
   ...
   */
  private async sendDiscussionStartMessage(
    channel: TextChannel,
    nominee: Nominee
  ): Promise<void> {
    try {
      ...
      const discussionStart = nominee.discussionStart
        ? new Date(nominee.discussionStart)
        : new Date();
      const discussionEnd = nominee.voteStart
        ? new Date(nominee.voteStart)
        : new Date(
  ...
  private async sendVoteStartMessage(
    ...
    try {
      ...
      const voteDurationFormatted =
        NOMINATION_CONFIG.VOTE_DURATION_MINUTES >= 1440
      ...
      // Get vote start time
      const voteStart = nominee.voteStart
        ? new Date(nominee.voteStart)
        : new Date();
      ...
      if (modCommsChannelId) {
        try {
          const modCommsChannel = channel.guild.channels.cache.get(
            modCommsChannelId
          ) as TextChannel;
