  ...
  private async transitionToCleanup(
    ...
    if (result.success) {
      ...
      if (voteResults) {
        ...
        // Vote period expired without readable poll data - skip posting results
        // (Don't post confusing all-zero results when poll data is unavailable)
        logger.warn(
          { nomineeId: nominee.id, nomineeName: nominee.name },
          'Vote period expired but poll results could not be read - skipping results post'
        );
      }
    } else {
      logger.error(
